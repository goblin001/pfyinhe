...
Install() {
	if [[ ${release} == "centos" ]]; then
		...
		wget https://github.com/snail007/goproxy/releases/download/v11.8/proxy-linux-amd64.tar.gz && tar -xzf proxy-linux-amd64.tar.gz proxy && mv -f /root/proxy /usr/bin/goproxy  && rm -f proxy-linux* && chmod +x /usr/bin/goproxy
